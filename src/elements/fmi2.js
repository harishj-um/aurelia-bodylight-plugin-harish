...
export class Fmi2 {
    ...
    initialize() {
      ...
      else {console.log('WARNING expected promise from EMSDK v2.x',); this.inst2 = this.inst; this.initialize1();}
    }
    ...
    startSimulation() {
      ...
      const performAnimation = () => {
        ...
        if (this.inst instanceof Promise) {this.inst.then(inst => this.step(inst));} else this.step();
      };
    ...
    step(inst) {
      ...
      if (!this.doingstep) {
        ...
        if (!this.instantiated) {
          console.log('instantiating before step()');
          if (inst) {
            let that = this;
            this.instantiate().then( () =>{
              that.instantiated = true;
              that.initialize();
              that.step1();
            });
          }
        ...
        console.log('WARNING another simulation step in progress.');
      }
    }
